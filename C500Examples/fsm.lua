...
local fsm = device.stateMachine { 'demoFSM', showState = true }
    ...
    .state { 'Zero', enter=function() device.write('topLeft', 'ZEROED') end }
    .state { 'Motion', short='MOVE', enter=function() device.write('topLeft', 'MOVING') end }
    ...
    .state { 'Nowhere', short='LOST', enter=function()
                device.write('topLeft', "I AM LOST SOMEWHERE ......", 'once, clear')
            end }
    .state { 'Somewhere', short='HERE', enter=function()
                device.write('topLeft', "I AM FOUND", 'once, clear')
            end }
    ...
    .trans { 'Start', 'Steady', status={'notzero', 'notmotion'},
                  activate=function() device.write('topLeft', 'PLEASE ZERO') end
            }
    .trans { 'Motion', 'Zero', status={'zero', 'notmotion'}, }
    .trans { 'Motion', 'Steady', status={'notzero', 'notmotion'},
                activate=function() device.write('topLeft', 'READY') end
            }
...
device.setKeyCallback('tare', function() fsm.raise('fin') return true end, 'short')

device.write('topLeft', 'FIND THE WAY OUT', 'wait, clear')
