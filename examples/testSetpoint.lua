...
local wgt = K401.addStream(K401.REG_GROSS, handleWeightStream, 'change')
...
local function startNext()
  ...
  if curGross >= target then
       curState = DUMP
       K401.setpIO(FILL_SETP,0)          -- remove output from the fill setpoint
       K401.setpIO(DUMP_SETP,DUMP_OUTPUT) -- enable DUMP output
       timeout = DUMP_TIMEOUT
       dbg.printVar('CurState = DUMP')
  else
     curState = FILL
     K401.setpIO(DUMP_SETP,0)           -- remove output from the dump setpoint
     K401.setpIO(FILL_SETP,FILL_OUTPUT) -- enable fill output 
...
local function abort()
  ...
  K401.setpIO(FILL_SETP,0)   
  K401.setpIO(DUMP_SETP,0)
...
local function resume()
 ...
 if curState == FILL then 
    if curGross >= target then       -- if paused after filling finished then move to dump
      ...
      K401.setpIO(FILL_SETP, FILL_OUTPUT)  -- otherwise enable fill output
    end  
 elseif curState == DUMP then
   if curGross <= empty then         -- if paused after dumping finished then move to fill 
      startNext()
   else   
      K401.setpIO(DUMP_SETP, DUMP_OUTPUT)  -- otherwise enable dump output
   end    
 end 
end
...
local function statusChanged(status, active)
   if status == K401.STAT_MOTION then
      if not active then                -- if weight stable
          if curState == FILL and curGross >= target then
             ...
             K401.printCustomTransmit(string.format('\\BF \\C0: Target = %s, Actual = %s\\C1',
   ...
   elseif status == K401.STAT_NOTERROR then  -- if weighing errors detected then pause
      if not active then 
          pause('Error')
      end
   end
...
K401.setStatusCallback(K401.STAT_NOTERROR, statusChanged)
     ...
     elseif key == K401.KEY_F1 then
           if paused then
            ...
            elseif curState == IDLE then
                ...
                tmp = K401.readReg(K401.setpRegAddress(FILL_SETP,K401.REG_SETP_TARGET)) 
                -- prompt user for target and start filling if OK pressed
                tmp, ok = K401.edit('TARGET',tmp,'number') 
                if ok then
                   K401.setpTarget(FILL_SETP,tmp)  -- save target value to setpoint 
                   -- read setpoint targets for local monitoring and logging
                   target = K401.readReg(K401.setpRegAddress(FILL_SETP,K401.REG_SETP_TARGET))  
                   empty = K401.readReg(K401.setpRegAddress(DUMP_SETP,K401.REG_SETP_TARGET))
                   startNext()  -- start filling process
                end
           end         
           return true  -- key handled in LUA
     -- F2 to Pause
     elseif key == K401.KEY_F2 and not paused then
            pause('OPERATOR')
            return true
     --F3 to abort
     elseif key == K401.KEY_F3 then
            abort()
            return true
     end
   end   
